...
const cardsS4Path = path.join(__dirname, "../src/data/wankulCardsS4.json");
const cardsS4 = JSON.parse(fs.readFileSync(cardsS4Path, "utf-8"));
...
const seed = async () => {
  try {
    ...
    for (const {
      ...
      booster_id,
      holo_mask,
    } of [...cards, ...cardsS2, ...cardsS3, ...cardsS4, ...cardsHS]) {
      await db.query(
